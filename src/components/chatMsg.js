...
const AvatarItem = props => {
  ...
  if (!avatar) {
    avatar =
      props.scene === 'p2p'
        ? configs.defaultUserIcon
        : configs.defaultGroupIcon;
  }
  return (
    <View
      style={{
...
const ChatContent = props => {
  ...
  if (msg.type === 'text') {
    ...
    return (
      <View
        ...
        {showTextArray.map((item, index) => {
          ...
          return (
            <Text key={id} style={chatStyle.text}>
              {item}
            </Text>
          );
        })}
    ...
    if (content.type === 1) {
      ...
      return <Text style={chatStyle.text}>[未知贴图表情]</Text>;
    }
...
export const ChatLeft = props => {
  ...
  return (
    <View style={[chatStyle.wrapper, chatStyle.left]}>
      <TouchableOpacity
        onPress={() => {
          props.navigation.navigate('namecard', {account: msg.from});
        }}>
          ...
          if (props.msgAction) {
            ...
            Alert.alert('提示', '只能撤回发送2分钟以内的消息，确认要撤回', [
              {text: '取消'},
              {
                text: '确认撤回',
                onPress: () => {
                  props.msgAction.backoutMsg(msg);
                },
              },
            ]);
          }
