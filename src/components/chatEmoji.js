      ...
      <View style={emojiStyle.emojiWrapper}>
        {emojis.list.map(item => (
          <TouchableOpacity
            ...
            onPress={() => {
              this.selectEmoji({key: item.key});
            }}>
            <Image
              source={item.img}
              key={item.key}
              style={{width: 6 * RVW, height: 6 * RVW}}
            />
          </TouchableOpacity>
        ))}
      </View>
        ...
        {pinups.list.map(item => (
          <TouchableOpacity
            ...
            <Image
              key={item.key}
              source={item.img}
              style={{width: 10 * RVW, height: 10 * RVW}}
            />
          </TouchableOpacity>
        ))}
      </View>
    ...
    return (
      <View style={[chatStyle.chatItemWraper, {paddingVertical: 0}]}>
        <ScrollView
          horizontal
          ref={scrollView => {
            this.emojiScrollView = scrollView;
          }}>
          {this.state.currType === 'emoji'
            ? this.renderEmoji()
            : this.renderPinup()}
        </ScrollView>
        <View
          style={[
            chatStyle.chatItemRow,
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  ...
                  {
                    backgroundColor:
                      this.state.currAlbum === name ? '#f0f0f0' : '#fff',
                  },
                ]}
                onPress={() => {
                  this.selectAlbum({type: 'emoji', name});
                }}>
                <Image
                  source={emoji.img}
                  style={{width: 6 * RVW, height: 6 * RVW}}
          ...
          {Object.keys(this.pinup).map(name => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => {
                  this.selectAlbum({type: 'pinup', name});
                }}>
                <Image
                  source={pinup.img}
                  style={{width: 6 * RVW, height: 6 * RVW}}
